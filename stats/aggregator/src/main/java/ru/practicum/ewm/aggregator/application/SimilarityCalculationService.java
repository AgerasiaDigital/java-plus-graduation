...
public class SimilarityCalculationService {
    ...
    public void updateSimilarities(long userId, long eventId, UserActionType actionType) {
        ...
        synchronized (userLock) {
            ...
            // Limit to top N events by weight to avoid excessive computation
            int maxEvents = recommendationProperties.maxUserEventsForSimilarity();
            if (userEventWeights.size() > maxEvents) {
                log.debug("User {} has {} events, limiting to top {} for similarity calculation.", userId, userEventWeights.size(), maxEvents);
                userEventWeights = userEventWeights.entrySet().stream()
            ...
            for (Long otherEventId : minWeightDeltas.keySet()) {
                ...
                double denominator = Math.sqrt(newWeightSum * otherEventWeightSum);
                if (denominator == 0.0) {
                    log.warn("Skipping similarity for pair ({}, {}): denominator is zero (newWeightSum={}, otherWeightSum={})",
                        eventA, eventB, newWeightSum, otherEventWeightSum);
                    continue;
                }
                double similarity = minWeightSum / denominator;
