...
public interface EventSimilarityRepository {
     ...
     * excluding events the user has already interacted with. The returned list is ordered by average similarity descending.
     *
     * @param recentlyInteractedEvents A {@link Collection} of event IDs representing the base set for similarity calculation.
     * @param excludedEvents A {@link Collection} of event IDs to exclude from results (e.g. already interacted events).
     ...
     */
    List<Long> findTopSimilarToSet(Collection<Long> recentlyInteractedEvents, Collection<Long> excludedEvents, int maxResults);
