...
public interface JpaEventSimilarityRepository extends EventSimilarityRepository, JpaRepository<EventSimilarity, Long> {
    ...
    @Override
    default List<Long> findTopSimilarToSet(Collection<Long> recentlyInteractedEvents,
        Collection<Long> excludedEvents, int maxResults) {
        if (recentlyInteractedEvents == null || recentlyInteractedEvents.isEmpty()) {
            return List.of();
        }
        Collection<Long> excluded = (excludedEvents == null || excludedEvents.isEmpty())
            ? List.of(-1L) : excludedEvents;
        Pageable limit = PageRequest.of(0, maxResults);
        return findTopSimilarToSetPageable(recentlyInteractedEvents, excluded, limit);
    }
    ...
    @Query(value = """
        ...
        WHERE ((es.eventA IN :eventSet AND es.eventB NOT IN :eventSet)
          ...
          AND (CASE WHEN es.eventA IN :eventSet THEN es.eventB ELSE es.eventA END NOT IN :excludedEvents)
