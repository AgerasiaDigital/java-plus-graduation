...
import ru.practicum.event.dto.ApiError;
...
public class ErrorHandler {
    ...
    @ResponseStatus(HttpStatus.BAD_REQUEST)
    public ApiError handleMissingRequestHeader(final MissingRequestHeaderException e) {
        log.warn("400 {}", e.getMessage());
        return new ApiError(e.getMessage(), "Missing required header",
    ...
    public ApiError handleValidationException(final ValidationException e) {
        log.warn("400 {}", e.getMessage());
        return new ApiError(e.getMessage(), "Invalid operation",
                HttpStatus.BAD_REQUEST.name(), LocalDateTime.now().format(formatter));
    }
    ...
    public ApiError handleAccessViolationException(final AccessViolationException e) {
        log.warn("409 {}", e.getMessage());
        return new ApiError(e.getMessage(), "Access violation",
                HttpStatus.CONFLICT.name(), LocalDateTime.now().format(formatter));
    }
    ...
    public ApiError handleIllegalStateException(final IllegalStateException e) {
        log.warn("409 {}", e.getMessage());
        return new ApiError(e.getMessage(), "Illegal state value",
                HttpStatus.CONFLICT.name(), LocalDateTime.now().format(formatter));
    }
